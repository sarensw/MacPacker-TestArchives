...
`zip/customicon.zip` is the fixture for a folder with a custom icon. Three separate pieces of metadata make that icon, and losing any one of them loses it: a zero-byte `Icon\r` file whose resource fork holds the `icns`, that file's `com.apple.FinderInfo` with kIsInvisible set — without which Finder shows it as `Icon?`, the shape MacPacker issue #216 was reported in — and the folder's own `com.apple.FinderInfo` with kHasCustomIcon, which is what makes Finder look for the icon file at all. Zip holds neither extended attributes nor resource forks, so all of it travels as AppleDouble the way `zip/appledouble.zip` describes; what earns this case a fixture of its own is that Finder's "Compress" does not carry all three. `ditto -c -k --sequesterRsrc --keepParent` writes a sidecar for `Icon\r` but none for the folder it was pointed at, so a round trip through Finder brings the icon file back — hidden, with its fork — and still shows a generic folder. This archive therefore holds the complete form, `__MACOSX/._CustomFolder` included: extracting it must leave no `__MACOSX` and no `._` file behind, `Icon\r` invisible with its resource fork, and the folder showing the picture again. The entry name is the other half of the fixture — `Icon\r` ends in a carriage return, which stock Info-ZIP `unzip` silently drops. Rebuild it with `zip/make_customicon.sh`; the resource fork is embedded and the entry dates and order are fixed, so a rebuild is byte-identical.
